...
async function applyBlogRLSFix() {
  try {
    ...
    const migrationPath = path.join(process.cwd(), 'supabase', 'migrations', '20250217000009_fix_blog_posts_rls.sql');
    ...
    if (error) {
      ...
      if (insertError) {
        ...
        console.log('💡 You may need to manually apply the RLS policies in your Supabase dashboard');
        console.log('📋 Go to: https://supabase.com/dashboard/project/bopzgxqujuqosdexnppj/editor');
        console.log('📋 Navigate to: Authentication > Policies');
        console.log('📋 Find the blog_posts table and add the policies from the migration file');
      } else {
        console.log('✅ Successfully inserted test post! RLS policies are working.');
